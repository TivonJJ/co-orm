...
function defineModel(name, properties, opts) {
	...
	defineCoRunMethod(Model,"find");
	defineCoRunMethod(Model,"all");
...
function extendInstanceWithAssociation(Instance, association) {
	...
	defineCoRunMethod(Instance,association.getAccessor);
...
function defineCoRunMethod(obj,method){
	var _m = obj[method];
	obj[method] = function(){
		var c = _m.apply(this,arguments);
		c.coRun = toThunkify(c.run,c,extendInstance);
		if(c.remove && !c.coRemove){
